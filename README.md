...
| 功能 | 说明 |
| --- | --- |
| ✨ AI 修复 | 内置 LaMa AI 模型本地推理，涂抹水印区域后一键修复；结果直接显示在原图窗口，可随时拖动滑块对比原图 |
...
3. 选择处理方式：
   - ✨ **AI 修复**：点「✨ AI 去水印」自动填充去除，结果直接显示在原图窗口，滑块默认停在结果端，向左拖动可对比原图
...
- **纯前端**：无服务器、无后端，静态托管即可
- **AI 修复**：内置 LaMa（int8 量化）修复模型 + `onnxruntime-web` WASM 本地推理；使用 ORT 1.18 非线程 WASM 构建（不依赖 SharedArrayBuffer，兼容 iOS Safari / 安卓 WebView），Worker 可用时自动走 Proxy 模式，否则回退主线程；先以 Telea 快速修复秒出预览，AI 完成后在原水印区域软边合成替换
- **AI 抠图**：内置 U²-Net（`u2netp`）分割模型（同样走非线程 WASM + Proxy 推理路径），实时生成前景遮罩，支持绿笔保留 / 红笔去除的交互式修正
...
- AI 模型首次使用会显示下载进度条，自动在多个镜像中选最快源并行加速（通常几十秒到几分钟），单个源无进展会自动切换，下载后缓存、离线复用；AI 推理使用非线程 WASM 构建，不依赖 SharedArrayBuffer（iOS Safari / 安卓 WebView 可用）；Worker 可用时在 Proxy 模式后台运行，否则回退主线程，单次修复通常需 1–3 分钟，期间会先显示快速修复结果
