...
- **AI 抠图**：内置 U²-Net（`u2netp`）分割模型，实时生成前景遮罩，支持绿笔保留 / 红笔去除的交互式修正
- **一键去水印**：四角连通域 + 对比度/饱和度分析定位豆包 AI / Gemini 水印，自动生成遮罩后走 AI 修复去除
- **马赛克**：涂抹时直接对画布区块做像素块化，橡皮恢复原图，所见即所得
- **模型加速下载**：模型拆分为 8 个分片，走 jsDelivr CDN 多路并行下载 + Range 分块，显示百分比/速度/剩余时间进度条，并缓存到 Cache API 离线复用
- **双指缩放**：Pointer Events 多指追踪，第二指落下取消首指未完成笔画、双指结束后防误涂；缩放使用 CSS transform，原始画布不重采样，缩回 1:1 画质无损
...
- AI 模型首次使用会显示下载进度条，走 CDN 并行加速（通常几十秒到几分钟），下载后缓存、离线复用；AI 修复单次通常需 1–3 分钟，期间会先显示快速修复结果
- 涂抹/框选尽量贴合水印轮廓，AI 修复效果更好
- iOS Safari 的视频输出为 MP4，安卓 Chrome 输出为 WebM（部分手机相册可能不识别 WebM，可用裁剪/马赛克模式再试或转码）

## ❓ 常见问题

- **AI 模型下载慢？** 已使用 jsDelivr CDN + 8 路并行下载加速；若你的网络访问 CDN 较慢，可尝试打开代理或更换网络后重新打开页面。
